...
class Interpeter(NodeVisitor):
    ...
    def _getObjType(self, el: LObject) -> str:
        # Returning the name of the type of the given LObject
    ...
    def visit_ProgramNode(self, node) -> None:
        ...
        :return: None
        """
        # Create and push the main frame to the call stack
        mainFrame = ActivationRecord(ARType.MAIN)
        self._callStack.push(mainFrame)
        self._curFrame = mainFrame
    ...
    def visit_ArrayNode(self, node) -> Array:
        ...
        for element_node in node.elements:
            arr.addEl(self.visit(element_node))
    ...
    def visit_ArrayAccessNode(self, node) -> LObject:
        ...
        # Everything is okay, return the accessed element from the array
    ...
    def visit_AddNode(self, node) -> Number:
        ...
        # Perform numeric addition if the left operand is a Number
            ...
            if self._getObjType(r) != "Number":
                raise TypeErr(f"Cannot add {self._getObjType(r)} to Number", node.right.token.line)
            return Number(l.value + r.value)

        # Raise an error if neither operand is a String or Number
        raise TypeErr(f"Invalid types for addition: {self._getObjType(l)} and {self._getObjType(r)}", node.left.token.line)
    ...
    def visit_GreaterThanNode(self, node) -> Boolean:
        ...
        # Comparison is only valid for numbers
        if self._getObjType(l) != "Number" or self._getObjType(r) != "Number":
    ...
    def visit_GreaterThanEqualNode(self, node) -> Boolean:
        ...
        This method checks if the left operand is greater than or equal to the right operand.
    ...
    def visit_NotNode(self, node) -> Boolean:
        ...
        return Boolean("false") if self._isTruthy(val) else Boolean("true")
    ...
    def visit_ConditionalNode(self, node) -> bool:
        ...
        # Return False indicating the condition is falsy and the statement is not executed
        return False
    ...
    def visit_IfNode(self, node) -> bool:
        ...
        # Return True if any if or elsif block is executed, False otherwise
        return res
